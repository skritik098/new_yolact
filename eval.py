...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_text or args.display_bboxes:
        ...
        cv2.putText(img_numpy, 'Number of Cars: {}'.format(num_cars), (860, 130), font, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
...
def prepare_out_blend_frame(blend_on_road, img_binary, img_birdeye, img_fit, line_lt, line_rt, offset_meter):
    ...
    cv2.putText(blend_on_road, 'Offset from center: {:.02f}m'.format(offset_meter), (860, 90), font, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
...
def evaLanevideo(net, path:str, out_path:str=None):
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    def get_next_frame(vid):
        ...
        for idx in range(args.video_multiframe):
            frame = cv2.cvtColor(process_pipeline(vid.read()[1], keep_state=False), code=cv2.COLOR_BGR2RGB)#vid.read()[1]
            if frame is None:
                return frames
            frames.append(frame)
        return frames
